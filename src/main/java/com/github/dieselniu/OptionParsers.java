...
class OptionParsers {
	...
	public static <T> OptionParser<T> unary(T defaultValue, Function<String, T> valueParser) {
		return ((arguments, option) -> getT(arguments, option, defaultValue, valueParser));
	}


	static <T> T getT(List<String> arguments, Option option, T value, Function<String, T> parser) {
		return values(arguments, option, 1).map(it -> parseValue(it.get(0), parser)).orElse(value);
	}
